...
<x-card>
    <x-slot name="header">
        <x-card-title> <span class="h-8 w-8 rounded-full bg-green-100 flex flex-col items-center justify-center mr-2 font-semibold text-lg text-green-500"> {{$key+1}}</span>Emergency Contact Person(s)</x-card-title>
        @error('list_of_emergency_contact_registered') <span class="error">{{ $message }}</span> @enderror
    </x-slot>
    <x-slot name="table">
...
<x-card>
    <x-slot name="header">
        ...
        @error('list_of_authorized') <span class="error">{{ $message }}</span> @enderror
    </x-slot>
    <x-slot name="table">
        <div class="bg-white grid grid-cols-2 mt-5">
            <div class="bg-white grid grid-cols-1">
...
<x-card>
    <x-slot name="actions">
        <button type="button" 
            wire:click="saveEmergencyContact()"
